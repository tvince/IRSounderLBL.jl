"""
Per-layer instrumentation of the worst point in the Julia−LBLRTM band-head
comb: the line core at 2390.235 cm⁻¹ (within the CO₂ ν₃ band-head where
many Q-branch lines overlap).

For each of the 49 layers we compute the TOTAL summed CO₂ cross-section at
exactly ν = 2390.235 cm⁻¹ (including all line wings within ±25 cm⁻¹), times
...
the line-core BT at exactly 2390.235 cm⁻¹.
...
CSV dump: data/lblrtm/instrument_2390_line.csv
...
const NU0     = 2390.235        # cm⁻¹
const CUTOFF  = 25.0
const OUT     = "data/lblrtm/instrument_2390_line.csv"
const LBL_CSV = "data/lblrtm/lblrtm_layers_43um.csv"

# Tiny single-point grid at exactly NU0
const NU_GRID = wavenumber_grid(NU0, NU0, 1.0)   # one point at 2390.235
...
function instrument(p_eff, T_eff, vmr_eff, N_CO2_layer, label)
    ...
    print("  computing per-layer σ(2390.235) and τ … ")
    t0 = time()
    for k in 1:n_lay
...
println("\n=== Summary: line core BT at 2390.235 cm⁻¹ ===")
