...
Matches arts_validation_cont_lm.py configuration:
  ...
  - CO2 VP_Y line mixing (HITRAN 2020 relaxation matrix, stot_min=0.0)
  - No ILS, 0.25 cm⁻¹ output grid

Uses the standard `iasi_forward_model` with `line_mixing=VPYLineMixing(relmat)`.
...
# Custom IASIInstrument over the 645–800 cm⁻¹ band (Δν=0.25 cm⁻¹, 621 channels)
...
println("Running iasi_forward_model with VPYLineMixing …")
t1 = time()
