"""
Cross-test: feed LBLRTM's per-layer τ at 2364.105 cm⁻¹ (from IOD=1 ODint
output) into Julia's Schwarzschild linear-in-τ RT and see what BT comes
out.  Compare against:
  - Julia native BT @ 2364.105 (forward model): 240.34 K
  - LBLRTM IEMIT=1 BT @ 2364.105 (interpolated): 233.25 K

Three predictions are produced:
  (a) Julia τ + Julia Schwarzschild + Julia profile T levels
       → should match the FWD model 240.34 K (sanity check)
...
Reads:
  data/lblrtm/instrument_2364_line.csv     -- Julia per-layer τ
  data/lblrtm/lblrtm_per_layer_tau_2364.csv-- LBLRTM per-layer τ (IOD=1)
...
function load_taus()
    J = readdlm("data/lblrtm/instrument_2364_line.csv", ',', header=true)
    hJ = String.(vec(J[2])); rJ = J[1]
    τ_jul = Float64.(rJ[:, findfirst(==("tau_layer_jul"), hJ)])

    L = readdlm("data/lblrtm/lblrtm_per_layer_tau_2364.csv", ',', header=true)
...
#   - or upstream τ/T_AVE     (then BT_cim ≈ LBLRTM 233.25 K — CIM is faithful).
...
println("=== BT at 2364.105 cm⁻¹ ===")
@printf("  (a) Julia τ    + Julia Toon RT                    = %.3f K   (FWD: 240.34)\n",
        bt_jul_jul_toon)
...
@printf("  (d) LBLRTM IEMIT=1 measurement (target)           ≈ 233.25 K   (Δc: %+.3f)\n",
        233.25 - bt_lbl_jul_cim)
println("\nInterpretation:")
