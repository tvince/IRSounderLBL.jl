...
function layer_optical_depth(ν_grid::WavenumberGrid,
    ...
    # ── CO₂ continuum: MT-CKD line-coupling + HITRAN CIA ────────────────
    k_co2_cont = co2_continuum(ν_grid, vmr_co2, p_hPa, T) .+
                 co2_cia(ν_grid, vmr_co2, p_hPa, T)
    τ_co2_cont = k_co2_cont .* dz_cm

    # ── N₂/O₂ CIA (dry-air fractions of the non-H₂O column) ─────────────
    vmr_dry = 1.0 - vmr_h2o
    τ_n2_cont = n2_cia(ν_grid, _N2_DRY_VMR * vmr_dry, p_hPa, T) .* dz_cm
    τ_o2_cont = o2_cia(ν_grid, _O2_DRY_VMR * vmr_dry, p_hPa, T) .* dz_cm
...
function layer_optical_depth(ν_grid::WavenumberGrid,
    ...
    τ .+= co2_cia(ν_grid, vmr_co2, p_hPa, T) .* dz_cm
    vmr_dry = 1.0 - vmr_h2o
    τ .+= n2_cia(ν_grid, _N2_DRY_VMR * vmr_dry, p_hPa, T) .* dz_cm
    τ .+= o2_cia(ν_grid, _O2_DRY_VMR * vmr_dry, p_hPa, T) .* dz_cm
